/**
 * 🪐 Project Terra - Dimensional Analysis Engine
 * Evaluates 5D base vector integrity [Mass, Length, Time, Current, Temperature] and reduces equations.
 */
...
export function addDimensions(v1, v2) {
    return v1.map((val, idx) => val + v2[idx]);
}

export function subtractDimensions(v1, v2) {
    return v1.map((val, idx) => val - v2[idx]);
}

export function multiplyDimension(v1, scalar) {
    return v1.map(val => val * scalar);
}

export function formatDimensionVector(v) {
