...
def fix_prose_corrupted_unicode(text: str) -> str:
    ...
    text = text.replace("∫ₚ", "\\oint")
    text = text.replace("⁰", "")
    text = text.replace("ₚ", "_p")
    text = text.replace("ᵥ", "_v")
    text = text.replace("ᵗ", "^t")
    text = text.replace("͡", "\\hbar")
    text = text.replace("θₗ", "\\Omega_\\Lambda")
    text = text.replace("ρₗ", "\\rho_\\Lambda")
    text = text.replace("ρₚᵥŧᵗ", "\\rho_{\\text{crit}}")
